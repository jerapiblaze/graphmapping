...
def CountUsedLinks_Solution(problem:GraphMappingProblem) -> int:
    ...
    for link in problem.PHY.edges:
        used_links = [k for k in problem.solution.keys() if str(k).endswith(f"({link[0]},_{link[1]})") and problem.solution[k]]
        link_count += 1 if len(used_links) else 0
    return link_count

def CountUsedNodes_Solution(problem:GraphMappingProblem) -> int:
    node_count = 0
    for node in problem.PHY.nodes:
        used_nodes = [k for k in problem.solution.keys() if str(k).endswith(f"{node}") and problem.solution[k]]
        node_count += 1 if len(used_nodes) else 0
    return node_count
...
def MpWorker(queue:mp.Queue, result_file:str):
    while queue.qsize():
        ...
        count_links = CountUsedLinks_Solution(solved_problem)
        count_nodes = CountUsedNodes_Solution(solved_problem)
        with open(result_file, "at") as f:
            f.write(f"{set_name},{solver_name},{problem_name},{status},{solution_status},{int(abs(obj_value))},{round(runtime,3)},{count_nodes},{count_links}\n")
    pass
...
def Main():
    ...
    with open(result_file, "wt") as f:
        f.write(f"setname,solvername,problemname,status,solutionstatus,objvalue,runtime,usednodescount,usedlinkscount\n")
