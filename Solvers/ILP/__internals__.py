...
class ILPSolver(GraphMappingSolver):
    ...
    def Solve(self) -> GraphMappingProblem:
        ...
        solution = {str(var): pulp.value(var) for var in self.ILP_PROBLEM.variables() if not pulp.value(
            var) == 0 and any(str(var).startswith(filter) for filter in VARIABLE_SURFIXES)}
