...
def Main(config):
    ...
    if config["DELETE_OLD_DATA"]:
        CleanDir("./data/__internals__/DQL2")
    ...
    for problem_path in problem_path_list:
        ...
        model_save_path = os.path.join("./data/__internals__/DQL2", f"{problem.name}.pkl.gz")
        DQL.agent3.SaveAgent(model_save_path, trained_agent)
        if not save_reward:
            continue
        rewards_save_path = os.path.join("./data/__internals__/DQL2", f"{problem.name}_rewards.csv")
...
if __name__=="__main__":
    config_list = ConfigParser("./configs/DQL2Settings/dummy.yaml")
